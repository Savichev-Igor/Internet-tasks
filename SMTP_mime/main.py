        ...
        for f in files:
            if len(re_images.findall(f)) > 0:
                ...
                with open(path+"/"+f, "rb") as normal_file:
                    encoded_string = base64.standard_b64decode(normal_file.read())
                    dict_images[f] = encoded_string

        return dict_images


    def image_b64(self, path):
        pass
...
def simple_socket(server, port, login, password):
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
def main():
    ...
    print(s.get_only_images_base64("C:/Users/Игорь/Desktop/Institute/Интернет (Практика)/Tasks/SMTP_mime/Pictures"))
    # s.send_mail()
    # s.connect_tls()
    ...
    # s.ehlo()
    # print(data)
    # simple_socket("smtp.mail.ru", 587, "Rick_Grimes72@mail.ru", "123test")
